...
- [ ] T052 Enhebrar la prop `puzzleId` por los **tres** archivos: `app/rooms/[code]/page.tsx` la toma de `board.puzzle.id` y se la pasa a `<Board>`, `components/Board.tsx` la reenvía a `<BoardCanvas>`, y `components/BoardCanvas.tsx` la recibe. La página no renderiza el canvas directamente. Es la semilla del generador: sin ella el componente no puede calcular ninguna forma
